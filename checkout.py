...
#         bias_files = hires.take_bias(n=nbiases)
    ...
    ##-------------------------------------------------------------------------
    if args.fake:
        dark_files = glob('/home/jwalawender/HIRES_Test_Data/*.fits')[10:]
    else:
        pass

    ndarks = len(dark_files)
    ## Determine Dark Stats for each Chip
    clipping_sigma = 5
    clipping_iters = 1
    plt.figure(figsize=(9,15), dpi=72)
    binsize = 1.0
